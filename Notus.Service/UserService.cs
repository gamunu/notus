...
namespace Notus.Service
{
    public interface IUserService
    {
        ...
        IEnumerable<ApplicationUser> GetInvitedUsers(string username, int groupId,
    ...
    public class UserService : IUserService
    {
        ...
        public IEnumerable<ApplicationUser> GetUsers(string username)
        {
            var users =
                userRepository.GetMany(
                    u => (u.FirstName + " " + u.LastName).Contains(username) || u.Email.Contains(username))
            ...
            IGroupInvitationService groupInvitationService)
        {
            return
                GetUsers(username)
                    .Join(
                        groupInvitationService.GetGroupInvitationsForGroup(groupId).Where(inv => inv.Accepted == false),
                        u => u.Id, i => i.ToUserId, (u, i) => u);
        }
        ...
        public IEnumerable<ApplicationUser> SearchUser(string searchString)
        {
            var users =
                userRepository.GetMany(
                    u =>
                        u.UserName.Contains(searchString) || u.FirstName.Contains(searchString) ||
                        u.LastName.Contains(searchString) || u.Email.Contains(searchString)).OrderBy(u => u.DisplayName);
            return users;
        }
