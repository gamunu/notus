...
namespace Notus.Service
{
    ...
    public class UpdateSupportService : IUpdateSupportService
    {
        ...
        {
            return
                UpdateSupportRepository.GetMany(s => s.UpdateId == updateId)
        ...
        {
            var oldUser =
                UpdateSupportRepository.GetMany(
                    g => g.UserId == support.UserId && g.UpdateSupportId == support.UpdateSupportId);
            if (oldUser.Count() == 0)
            {
