...
namespace Notus.Service
{
    ...
    public class GroupUpdateSupportService : IGroupUpdateSupportService
    {
        ...
        public GroupUpdateSupportService(IGroupUpdateSupportRepository GroupUpdateSupportRepository,
        ...
        {
            return
                GroupUpdateSupportRepository.GetMany(s => s.GroupUpdateId == updateId)
                    .OrderByDescending(s => s.UpdateSupportedDate);
        }
        ...
        //    return supports;
        //}
        public void CreateUserSupport(GroupUpdateSupport support)
            //, ISupportInvitationService supportInvitationService)
        {
            var oldUser =
                GroupUpdateSupportRepository.GetMany(
                    g => g.GroupUserId == support.GroupUserId && g.GroupUpdateSupportId == support.GroupUpdateSupportId);
            if (oldUser.Count() == 0)
            {
        ...
        public bool IsUpdateSupported(int updateid, string userid, IGroupUserService groupUserService)
        {
            ...
            return GroupUpdateSupportRepository.Get(g => g.GroupUpdateId == updateid && g.GroupUserId == groupuserid) !=
                   null;
        }
