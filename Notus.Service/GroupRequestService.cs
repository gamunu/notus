...
namespace Notus.Service
{
    ...
    public class GroupRequestService : IGroupRequestService
    {
        ...
        {
            var oldgroup =
                GetGroupRequests().Where(g => g.UserId == groupRequest.UserId && g.GroupId == groupRequest.GroupId);
            if (oldgroup.Count() == 0)
            {
        ...
        {
            var groupRequests =
                groupRequestRepository.GetMany(g => g.UserId == userId && g.GroupId == groupId && g.Accepted == false);
