...
namespace Notus.Service
{
    ...
    public class CommentService : ICommentService
    {
        ...
        public CommentService(ICommentRepository commentRepository, ICommentUserRepository commentUserRepository,
            IUnitOfWork unitOfWork, IFollowUserRepository followUserRepository)
        {
        ...
        public IEnumerable<Comment> GetCommentsByUser(string userid)
        {
            var comments =
                from c in
                    commentRepository.GetMany(c => c.Update.Goal.GoalType == false)
        ...
        public IEnumerable<Comment> GetTop20CommentsOfPublicGoalFollwing(string userId)
        {
            ...
            var commentUsers = (from f in followUserRepository.GetMany(fol => fol.FromUserId == userId)
                join com in (from cu in commentUserRepository.GetAll()
                    select cu) on f.ToUserId equals com.UserId
                select com).ToList();
