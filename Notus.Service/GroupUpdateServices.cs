...
namespace Notus.Service
{
    ...
    public class GroupUpdateService : IGroupUpdateService
    {
        ...
        public GroupUpdateService(IGroupUpdateRepository updateRepository,
            IGroupUpdateUserRepository groupUpdateUserRepository, IGroupGoalRepository groupGoalRepository,
            IUnitOfWork unitOfWork)
        {
            groupUpdateRepository = updateRepository;
        ...
        {
            var updates = from u in groupUpdateRepository.GetAll()
                where
                    (from g in groupUserService.GetGroupUsers() where g.UserId == userid select g.GroupId).ToList()
                        .Contains(u.GroupGoal.GroupUser.GroupId)
                select u;
            return updates;
        }
        ...
        {
            var updates =
                groupUpdateRepository.GetMany(u => u.GroupGoalId == goalid)
                    .OrderByDescending(u => u.GroupUpdateId)
                    .ToList();
            return updates;
        }
        ...
        {
            var updates =
                groupUpdateRepository.GetMany(u => (u.GroupGoalId == goalid) && (u.status != null))
                    .OrderByDescending(u => u.GroupUpdateId)
                    .ToList();
            return updates;
        }
