...
namespace Notus.Service
{
    public interface IGroupUserService
    {
        ...
        IEnumerable<ApplicationUser> SearchUserForGroup(string searchString, int groupId, IUserService userService,
    ...
    public class GroupUserService : IGroupUserService
    {
        ...
        public GroupUserService(IGroupUserRepository groupUserRepository, IUserRepository userRepository,
        ...
        public IEnumerable<int> GetGroupUsers(string userid)
        {
            ...
            var groupUsers =
                groupUserRepository.GetMany(g => g.UserId == userid).OrderByDescending(g => g.GroupId).ToList();
        ...
        public IEnumerable<int> GetGroupAdminUsers(string userid)
        {
            var groupIds = new List<int>();
            var groupUsers =
                groupUserRepository.GetMany(g => (g.UserId == userid) && g.Admin)
                    .OrderByDescending(g => g.GroupUserId)
                    .ToList();
            foreach (var item in groupUsers)
            {
        ...
        {
            var users =
                groupUserRepository.GetMany(g => g.UserId == userid)
                    .OrderByDescending(g => g.GroupUserId)
                    .Take(20)
                    .ToList();
        ...
        {
            var groupUsers =
                groupUserRepository.GetMany(g => g.GroupId == groupId).OrderByDescending(g => g.GroupUserId).ToList();
            return groupUsers;
        }
        ...
        {
            var groupUsers =
                groupUserRepository.GetMany(g => g.GroupId == groupId && g.UserId != currentUserId)
                    .OrderByDescending(g => g.GroupUserId)
                    .ToList();
            return groupUsers;
        }

        public IEnumerable<ApplicationUser> SearchUserForGroup(string searchString, int groupId,
            IUserService userService, IGroupInvitationService groupInvitationService)
        {
        ...
        public IEnumerable<int> GetFollowedGroups(string userid)
        {
            var groupIds = new List<int>();
            var groupUsers =
                groupUserRepository.GetMany(g => (g.UserId == userid) && (g.Admin == false))
                    .OrderByDescending(g => g.GroupUserId)
                    .ToList();
            foreach (var item in groupUsers)
            {
        ...
        public IEnumerable<ApplicationUser> GetMembersOfGroup(int groupId)
        {
            ...
            var users = from u in userRepository.GetAll()
                join gu in
                    (from g in groupUserRepository.GetMany(gr => gr.GroupId == groupId) select g) on u.Id equals
                    gu.UserId
                select u;
            return users;
        }
        ...
        {
            var oldUser =
                groupUserRepository.GetMany(g => g.UserId == groupUser.UserId && g.GroupId == groupUser.GroupId);
            if (oldUser.Count() == 0)
            {
        ...
        public IEnumerable<int> GetGroupUsers(IEnumerable<string> userid)
        {
            ...
            foreach (var item in userid)
            {
                ...
                foreach (var gruser in groupUsers)
                {
        ...
        {
            return groupUserRepository.Get(g => g.GroupId == groupId && g.Admin).UserId;
        }
        ...
        public IEnumerable<string> GetUserIdByGroupUserId(IEnumerable<int> groupuserid)
        {
            ...
            foreach (var item in groupuserid)
            {
