...
namespace Notus.Service
{
    public interface ISupportService
    {
        ...
        IEnumerable<ApplicationUser> SearchUserToSupport(string searchString, int goalId, IUserService userService,
    ...
    public class SupportService : ISupportService
    {
        ...
        public SupportService(ISupportRepository SupportRepository, IFollowUserRepository followUserRepository,
        ...
        public IEnumerable<Support> GetTop20SupportsOfFollowings(string userId)
        {
            ...
            var supports = (from s in SupportRepository.GetMany(s => s.Goal.GoalType == false)
                where
                    (from f in followUserRepository.GetMany(fol => fol.FromUserId == userId) select f.ToUserId).ToList()
                        .Contains(s.UserId)
                select s).OrderByDescending(s => s.SupportedDate).Take(20);
            return supports;
        }
        ...
        {
            var supports =
                SupportRepository.GetMany(s => (s.Goal.GoalType == false) && (s.UserId == userid))
                    .OrderByDescending(s => s.SupportedDate)
                    .Take(20)
                    .ToList();
            return supports;
        }
        ...
        public IEnumerable<Goal> GetUserSupportedGoals(string userid, IGoalService goalService)
        {
            return
                GetSupports()
                    ...
                    .Join(goalService.GetGoals(), s => s.GoalId, g => g.GoalId, (s, g) => g)
        ...
        {
            var id =
                (from s in GetSupports() where s.GoalId == goalid && s.UserId == userid select s.SupportId)
                    .FirstOrDefault();
        ...
        public IEnumerable<ApplicationUser> SearchUserToSupport(string searchString, int goalId,
            IUserService userService, ISupportInvitationService supportInvitationService, string userId)
        {
