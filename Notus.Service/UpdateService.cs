...
namespace Notus.Service
{
    ...
    public class UpdateService : IUpdateService
    {
        ...
        public UpdateService(IUpdateRepository updateRepository, IGoalRepository goalRepository, IUnitOfWork unitOfWork,
        ...
        {
            var updates =
                updateRepository.GetMany(u => (u.Goal.UserId == userid && u.Goal.GoalType == false))
        ...
        {
            var updates = from u in updateRepository.GetMany(u => (u.Goal.GoalType == false))
                where
                    (from f in followUserRepository.GetMany(fol => fol.FromUserId == userid) select f.ToUserId).ToList()
                        .Contains(u.Goal.UserId)
                select u;
            return updates;
        }
        ...
        {
            var updates =
                updateRepository.GetMany(u => (u.Goal.GoalType == false) && (u.Goal.UserId == userid))
                    .OrderByDescending(u => u.UpdateDate)
                    .Take(20)
                    .ToList();
            ;
        ...
        public Update GetHighestUpdateValue(int goalid)
        {
            try
            {
                var updates = updateRepository.GetMany(u => u.GoalId == goalid).OrderByDescending(u => u.status).First();
                return updates;
            }
            catch
            {
        ...
        {
            var updates =
                updateRepository.GetMany(u => (u.GoalId == goalid) && (u.status != null))
                    .OrderByDescending(u => u.UpdateId)
                    .ToList();
            return updates;
        }
