...
namespace Notus.Service
{
    ...
    public class GroupService : IGroupService
    {
        ...
        public GroupService(IGroupRepository groupRepository, IFollowUserRepository followUserrepository,
        ...
        {
            return
                groupRepository.GetMany(g => g.GroupName.ToLower().Contains(group.ToLower())).OrderBy(g => g.GroupName);
        }
        ...
        public IPagedList<Group> GetGroups(string userId, GroupFilter filter, Page page)
        {
            switch (filter)
            {
                ...
                {
                    var groupsIds =
                        groupUserrepository.GetMany(gru => gru.UserId == userId && gru.Admin).Select(gru => gru.GroupId);
                    return groupRepository.GetPage(page, where => groupsIds.Contains(where.GroupId),
                ...
                case GroupFilter.MyFollowingsGroups:
                {
                    ...
                    var groupIds = from item in userIds
                        from gruser in groupUserrepository.GetMany(g => g.UserId == item)
                        select gruser.GroupId;
                    return groupRepository.GetPage(page, where => groupIds.Contains(where.GroupId),
                        order => order.CreatedDate);
                }
