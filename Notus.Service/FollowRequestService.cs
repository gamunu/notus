...
namespace Notus.Service
{
    ...
    public class FollowRequestService : IFollowRequestService
    {
        ...
        public void CreateFollowRequest(FollowRequest followRequest)
        {
            var oldfollow =
                GetFollowRequests()
                    .Where(g => g.ToUserId == followRequest.ToUserId && g.FromUserId == followRequest.FromUserId);
        ...
        {
            var followRequests =
                followRequestRepository.GetMany(
                    g => (g.FromUserId == fromuserId && g.ToUserId == touserid && g.Accepted == false));
