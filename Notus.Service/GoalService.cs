...
namespace Notus.Service
{
    ...
    public class GoalService : IGoalService
    {
        ...
        public GoalService(IGoalRepository goalRepository, IFollowUserRepository followUserRepository,
        ...
        {
            var goal =
                goalRepository.GetMany(g => (g.UserId == userid && g.GoalType == false))
        ...
        {
            var goals = (from g in goalRepository.GetMany(g => g.GoalType == false)
                where
                    (from f in followUserrepository.GetMany(fol => fol.FromUserId == userid) select f.ToUserId).ToList()
                        .Contains(g.UserId)
                select g).OrderByDescending(g => g.CreatedDate);
            return goals;
        }
        ...
        public IEnumerable<Goal> GetGoalsofFollowingByPopularity(string userid)
        {
            var goals = (from g in goalRepository.GetMany(g => g.GoalType == false)
                where
                    (from f in followUserrepository.GetMany(fol => fol.FromUserId == userid) select f.ToUserId).ToList()
                        .Contains(g.UserId)
                select g).OrderByDescending(g => g.Supports.Count());
            return goals;
        }
        ...
        {
            var goals =
                goalRepository.GetMany(g => g.GoalType == false).OrderByDescending(g => g.Supports.Count()).ToList();
            return goals;
        }
        ...
        public IEnumerable<Goal> GetGoalsbyPopularity(string userid)
        {
            var goals =
                goalRepository.GetMany(g => (g.UserId == userid && g.GoalType == false))
                    .OrderByDescending(g => g.Supports.Count())
                    .ToList();
            return goals;
        }
        ...
        {
            var goals =
                goalRepository.GetMany(g => (g.GoalType == false) && (g.UserId == userid))
                    .OrderByDescending(g => g.CreatedDate)
                    .Take(20)
                    .ToList();
            return goals;
        ...
        {
            return
                goalRepository.GetMany(g => g.GoalName.ToLower().Contains(goal.ToLower()) && g.GoalType == false)
        ...
        public IEnumerable<ValidationResult> CanAddGoal(Goal newGoal, IUpdateService updateService)
        {
            ...
            {
                var Updates =
                    updateService.GetUpdatesByGoal(newGoal.GoalId).OrderByDescending(g => g.UpdateDate).ToList();
                if (Updates.Count() > 0)
                {
                    ...
                    {
                        yield return
                            new ValidationResult("EndDate",
                                Resources.EndDateNotValid + " " + Updates[0].UpdateDate.ToString("dd-MMM-yyyy"));
                    }
                    else if (status == 1)
                    {
                        yield return
                            new ValidationResult("StartDate",
                                Resources.StartDate + " " + Updates[0].UpdateDate.ToString("dd-MMM-yyyy"));
                    }
                }
            }
        }
        ...
        public IEnumerable<Goal> GetGoalsByPage(string userId, int currentPage, int noOfRecords, string sortBy,
            string filterBy)
        {
