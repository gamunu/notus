...
namespace Notus.Service
{
    ...
    public class FollowUserService : IFollowUserService
    {
        ...
        {
            var oldUser =
                followUserRepository.GetMany(
                    g => g.FromUserId == followUser.FromUserId && g.ToUserId == followUser.ToUserId);
            if (oldUser.Count() == 0)
            {
        ...
        {
            var user = from f in followUserRepository.GetAll()
                where
                    (from g in followUserRepository.GetMany(fol => fol.FromUserId == userid) select g.ToUserId).ToList()
                        .Contains(f.FromUserId)
                select f;
            return user;
        }
