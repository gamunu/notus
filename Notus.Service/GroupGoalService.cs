...
namespace Notus.Service
{
    ...
    public class GroupGoalService : IGroupGoalService
    {
        ...
        {
            var goals = from g in GroupGoalRepository.GetAll()
                where
                    (from gu in groupUserService.GetGroupUsers() where gu.UserId == userid select gu.GroupId).ToList()
                        .Contains(g.GroupUser.GroupId)
                select g;
            return goals;
        }
        ...
        public IEnumerable<ValidationResult> CanAddGoal(GroupGoal newGoal, IGroupUpdateService groupUpdateService)
        {
            ...
            if (newGoal.GroupGoalId == 0)
                ...
                goal =
                    GroupGoalRepository.Get(
                        g =>
                            (g.GroupId == newGoal.GroupId) && (g.GoalName == newGoal.GoalName) &&
                            g.GroupGoalId != newGoal.GroupGoalId);
            if (goal != null)
            {
            ...
            {
                var Updates =
                    groupUpdateService.GetUpdatesByGoal(newGoal.GroupGoalId)
        ...
        {
            var goals =
                GroupGoalRepository.GetMany(f => ((f.GroupUserId == userid) && (f.AssignedGroupUserId != null)))
                    .ToList();
            return goals;
        }
