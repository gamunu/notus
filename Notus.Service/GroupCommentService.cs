...
namespace Notus.Service
{
    ...
    public class GroupCommentService : IGroupCommentService
    {
        ...
        public GroupCommentService(IGroupCommentRepository groupCommentRepository,
            IGroupCommentUserRepository groupCommentUserRepository, IGroupUpdateRepository groupUdateRepository,
        ...
        {
            var comment = from u in groupCommentRepository.GetAll()
                where
                    (from g in groupUserService.GetGroupUsers() where g.UserId == userid select g.GroupId).ToList()
                        .Contains(u.GroupUpdate.GroupGoal.GroupUser.GroupId)
                select u;
