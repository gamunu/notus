...
namespace Notus.Data.Repository
{
    public class GoalRepository : RepositoryBase<Goal>, IGoalRepository
    {
            ...
            string filterBy)
        {
            ...
            var goals = GetMany(g => g.GoalType == false);
            ...
            if (filterBy == "My Followings Goals")
            {
                goals = from g in goals
                    where
                        (from f in DataContext.FollowUser.Where(fol => fol.FromUserId == userId) select f.ToUserId)
                            .ToList().Contains(g.UserId)
                    select g;
            }
            //User goals
            ...
            {
                goals = from g in goals
                    join s in DataContext.Support on g.GoalId equals s.GoalId
