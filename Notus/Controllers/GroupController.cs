...
namespace Notus.Controllers
{
    ...
    public class GroupController : Controller
    {
        ...
        public GroupController(IGroupService groupService, IGroupUserService groupUserService, IUserService userService,
            IMetricService metricService, IFocusService focusService, IGroupGoalService groupgoalService,
            IGroupInvitationService groupInvitationService, ISecurityTokenService securityTokenService,
            IGroupUpdateService groupUpdateService, IGroupCommentService groupCommentService,
            IGoalStatusService goalStatusService, IGroupRequestService groupRequestService,
            IFollowUserService followUserService, IGroupCommentUserService groupCommentUserService,
            IGroupUpdateSupportService groupUpdateSupportService, IGroupUpdateUserService groupUpdateUserService)
        {
        ...
        public IUserMailer UserMailer { get; set; } = new UserMailer();
        ...
        {
            var group = Mapper.Map<Group, GroupViewModel>(groupService.GetGroup(id));
            group.Goals =
                Mapper.Map<IEnumerable<GroupGoal>, IEnumerable<GroupGoalViewModel>>(groupGoalService.GetGroupGoals(id));
            ...
            {
                group.GoalsAssignedToOthers =
                    Mapper.Map<IEnumerable<GroupGoal>, IEnumerable<GroupGoalViewModel>>(
                        groupGoalService.GetAssignedGoalsToOthers(assignedgroupuser.GroupUserId));
                group.GoalsAssignedToMe =
                    Mapper.Map<IEnumerable<GroupGoal>, IEnumerable<GroupGoalViewModel>>(
                        groupGoalService.GetAssignedGoalsToMe(assignedgroupuser.GroupUserId));
            }
            group.Focus = focusService.GetFocussOFGroup(id);

            //group.GroupUserId = groupUserService.GetGroupUserByuserId(((NotusUser)(User.Identity)).UserId).GroupUserId;
            group.Users = groupUserService.GetMembersOfGroup(id);
            //if (group.GroupUser.UserId == ((NotusUser)(User.Identity)).UserId)
        ...
        public ViewResult JoinedUsers(int id)
        {
            ...
            var gmvm = new GroupMemberViewModel {GroupId = id, Group = groupService.GetGroup(id), Users = members};
            return View(gmvm);
        }
        ...
        public ViewResult ShowAllRequests(int id)
        {
            ...
            var groupRequestViewModel =
                Mapper.Map<IEnumerable<GroupRequest>, IEnumerable<GroupRequestViewModel>>(groupRequests);
        ...
        public ActionResult CreateGroup(GroupFormModel newGroup)
        {
            ...
            var group = Mapper.Map<GroupFormModel, Group>(newGroup);
            var errors = groupService.CanAddGroup(group).ToList();
            ModelState.AddModelErrors(errors);
            if (ModelState.IsValid)
            {
                //group.UserId = ((NotusUser)(User.Identity)).UserId;
                var createdGroup = groupService.CreateGroup(group, userId);
                //var createdGroup = groupService.GetGroup(newGroup.GroupName);
                //var groupAdmin = new GroupUser { GroupId = createdGroup.GroupId, UserId = ((NotusUser)(User.Identity)).UserId, Admin = true };
        ...
        public ActionResult EditGroup(int id)
        {
            ...
            var editGroup = Mapper.Map<Group, GroupFormModel>(group);
            if (group == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult CreateFocus(FocusFormModel focus)
        {
            ...
            if (ModelState.IsValid)
            {
                var newFocus = Mapper.Map<FocusFormModel, Focus>(focus);
        ...
        {
            var Focus = Mapper.Map<Focus, FocusViewModel>(focusService.GetFocus(id));
            Focus.GroupGoal =
                Mapper.Map<IEnumerable<GroupGoal>, IEnumerable<GroupGoalViewModel>>(
                    groupGoalService.GetGroupGoalsByFocus(id));
        ...
        public ActionResult EditFocus(int id)
        {
            ...
            var editFocus = Mapper.Map<Focus, FocusFormModel>(Focus);
            if (Focus == null)
            {
                return HttpNotFound();
            }
        ...
        {
            var focus = Mapper.Map<FocusFormModel, Focus>(focusFormViewModel);
        ...
        {
            var group = Mapper.Map<GroupFormModel, Group>(groupFormViewModel);
            var errors = groupService.CanAddGroup(group).ToList();
            ModelState.AddModelErrors(errors);
            if (ModelState.IsValid)
            {
        ...
        {
            var groupgoal = Mapper.Map<GroupGoalFormModel, GroupGoal>(goal);
            ...
            if (ModelState.IsValid)
            {
                //groupgoal.GroupUser.UserId = ((NotusUser)(User.Identity)).UserId;
        ...
        public ViewResult EditGoal(int id)
        {
            ...
            var editGoal = Mapper.Map<GroupGoal, GroupGoalFormModel>(goal);
            var metrics = metricService.GetMetrics();
        ...
        {
            var groupGoal = Mapper.Map<GroupGoalFormModel, GroupGoal>(editGoal);
        ...
        public ActionResult SaveUpdate(GroupUpdateFormModel newupdate)
        {
            if (ModelState.IsValid)
            {
                var update = Mapper.Map<GroupUpdateFormModel, GroupUpdate>(newupdate);
                var userId = User.Identity.GetUserId();
                groupUpdateService.CreateUpdate(update, userId);
                var Updates =
                    Mapper.Map<IEnumerable<GroupUpdate>, IEnumerable<GroupUpdateViewModel>>(
                        groupUpdateService.GetUpdatesByGoal(newupdate.GroupGoalId));
                foreach (var item in Updates)
                {
                    item.IsSupported = groupUpdateSupportService.IsUpdateSupported(item.GroupUpdateId,
                        User.Identity.GetUserId(), groupUserService);
                    item.UserId = groupUpdateUserService.GetGroupUpdateUser(item.GroupUpdateId).Id;
                }
                var updates = new GroupUpdateListViewModel
                {
        ...
        public PartialViewResult DisplayUpdates(int id)
        {
            var Updates =
                Mapper.Map<IEnumerable<GroupUpdate>, IEnumerable<GroupUpdateViewModel>>(
                    groupUpdateService.GetUpdatesByGoal(id));
        ...
        public PartialViewResult DisplayComments(int id)
        {
            ...
            var commentsView = Mapper.Map<IEnumerable<GroupComment>, IEnumerable<GroupCommentsViewModel>>(comments);
        ...
        {
            var commentcount = groupCommentService.GetCommentsByUpdate(id).Count();
        ...
        public JsonResult SearchUserForGroup(string username, int groupId)
        {
            return
                Json(
                    from g in
                        groupUserService.SearchUserForGroup(username, groupId, userService, groupInvitationService)
        ...
        {
            var group = groupService.GetGroup(id);
            var invGroup = Mapper.Map<Group, GroupViewModel>(group);
        ...
        public string GoalStatus(int id, int goalid)
        {
            ...
            var msg = goal.GoalStatus.GoalStatusType;
            return msg;
        }
        ...
        public ActionResult EditUpdate(int id)
        {
            ...
            var editUpdate = Mapper.Map<GroupUpdate, GroupUpdateFormModel>(update);
            if (update == null)
            {
                return HttpNotFound();
            }
        ...
        public void SupportUpdate(int id)
        {
            ...
            groupUpdateSupportService.CreateSupport(new GroupUpdateSupport
            {
                GroupUserId = groupuser.GroupUserId,
                GroupUpdateId = id,
                UpdateSupportedDate = DateTime.Now
            });
        }
        ...
        {
            var supportcount = groupUpdateSupportService.GetSupportcount(id);
        ...
        public PartialViewResult SupportersOfUpdate(int id)
        {
            ...
            var usvm = new GroupUpdateSupportersViewModel
            {
                GroupUpdateId = id,
                GroupUpdate = groupUpdateService.GetUpdate(id),
                Users = supporters
            };
