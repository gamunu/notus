...
namespace Notus.Controllers
{
    ...
    public class GoalController : Controller
    {
        ...
        public GoalController(IGoalService goalService, IMetricService metricService, IFocusService focusService,
            ISupportService supportService, IUpdateService updateService, ICommentService commentService,
            IUserService userService, ISecurityTokenService securityTokenService,
            ISupportInvitationService supportInvitationService, IGoalStatusService goalStatusService,
            ICommentUserService commentUserService, IUpdateSupportService updateSupportService)
        {
        ...
        {
            var goal = Mapper.Map<GoalFormModel, Goal>(createGoal);
        ...
        {
            var goal = goalService.GetGoal(id);
            var editGoal = Mapper.Map<Goal, GoalFormModel>(goal);
            if (goal == null)
            {
                return HttpNotFound();
            }
        ...
        {
            var goalToEdit = Mapper.Map<GoalFormModel, Goal>(editGoal);
        ...
        {
            var Updates =
                Mapper.Map<IEnumerable<Update>, IEnumerable<UpdateViewModel>>(updateService.GetUpdatesByGoal(id));
            foreach (var item in Updates)
            {
                item.IsSupported = updateSupportService.IsUpdateSupported(item.UpdateId, User.Identity.GetUserId());
            }
            var updates = new UpdateListViewModel
            {
                Updates = Updates,
                Metric = goalService.GetGoal(id).Metric,
                Target = goalService.GetGoal(id).Target
                // IsSupported =  updateSupportService.IsUpdateSupported(,((NotusUser)(User.Identity)).UserId)
            };
        ...
        public ViewResult Supporters(int id)
        {
            ...
            var gsvm = new GoalSupporterViewModel {GoalId = id, Goal = goalService.GetGoal(id), Users = members};
            return View(gsvm);
        }
        ...
        public ActionResult SaveUpdate(UpdateFormModel newupdate)
        {
            ...
            if (ModelState.IsValid)
            {
                var update = Mapper.Map<UpdateFormModel, Update>(newupdate);
                ...
                var Updates =
                    Mapper.Map<IEnumerable<Update>, IEnumerable<UpdateViewModel>>(
                        updateService.GetUpdatesByGoal(newupdate.GoalId));
                foreach (var item in Updates)
                {
                    item.IsSupported = updateSupportService.IsUpdateSupported(item.UpdateId, User.Identity.GetUserId());
                }
                var updates = new UpdateListViewModel
                {
                    ...
                    //IsSupported = updateSupportService.IsUpdateSupported(newupdate.UpdateId,((NotusUser)(User.Identity)).UserId)
                };
                return PartialView("_UpdateView", updates);
            }
            return null;
        ...
        {
            var newInvitation = new SupportInvitation
            {
                GoalId = id,
        ...
        public PartialViewResult DisplayComments(int updateId)
        {
            ...
            var commentsView = Mapper.Map<IEnumerable<Comment>, IEnumerable<CommentsViewModel>>(comments);
        ...
        {
            var commentcount = commentService.GetCommentsByUpdate(UpdId).Count();
        ...
        public JsonResult SearchUser(string username, int goalId)
        {
            return
                Json(
                    from g in
                        supportService.SearchUserToSupport(username, goalId, userService, supportInvitationService,
        ...
        public IEnumerable<Goal> SearchGoal(string name)
        {
            ...
            goals.ForEach(
                g =>
                    g.Supported =
                        g.UserId != User.Identity.GetUserId()
                            ? (bool?) supportService.IsGoalSupported(g.GoalId, User.Identity.GetUserId())
                            : null);
        ...
        {
            supportService.CreateUserSupport(
                new Support {UserId = User.Identity.GetUserId(), GoalId = id, SupportedDate = DateTime.Now},
        ...
        public ActionResult EditUpdate(int id)
        {
            ...
            var editUpdate = Mapper.Map<Update, UpdateFormModel>(update);
            if (update == null)
            {
                return HttpNotFound();
            }
        ...
        {
            updateSupportService.CreateSupport(new UpdateSupport
            {
                UserId = User.Identity.GetUserId(),
                UpdateId = id,
        ...
        {
            var supportcount = updateSupportService.GetSupportcount(id);
        ...
        public PartialViewResult SupportersOfUpdate(int id)
        {
            ...
            var usvm = new UpdateSupportersViewModel
            {
                UpdateId = id,
                Update = updateService.GetUpdate(id),
                Users = supporters
            };
