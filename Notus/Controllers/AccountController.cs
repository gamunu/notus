...
namespace Notus.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(IUserService userService, IUserProfileService userProfileService,
            IGoalService goalService, IUpdateService updateService, ICommentService commentService,
            IFollowRequestService followRequestService, IFollowUserService followUserService,
            ISecurityTokenService securityTokenService, UserManager<ApplicationUser> userManager)
        {
        ...
        public ActionResult Login(string returnUrl)
        {
            if (Request.QueryString["guid"] != null)
                NotusSessionFacade.JoinGroupOrGoal = Request.QueryString["guid"];
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _userProfileService.CreateUserProfile(userId);
                    await SignInAsync(user, false);
                    if (NotusSessionFacade.JoinGroupOrGoal != null)
                    {
                        var groupOrGoalJoinToken = NotusSessionFacade.JoinGroupOrGoal;
                        if (groupOrGoalJoinToken.StartsWith("gr:"))
                        {
                            var groupIdToken = new Guid(groupOrGoalJoinToken.Substring(3));
                            TempData["grToken"] = groupIdToken;
                            return RedirectToAction("AddGroupUser", "EmailRequest");
                        }
                        if (groupOrGoalJoinToken.StartsWith("go:"))
                        {
                            var goalIdToken = new Guid(groupOrGoalJoinToken.Substring(3));
        ...
        {
            var users = _userService.SearchUser(name);
            return users;
        }
        ...
        public ActionResult ImageUpload()
        {
            ...
            imageVM.LocalPath = _userService.GetUser(User.Identity.GetUserId()).ProfilePicUrl;
        ...
        public ActionResult UploadImage(UploadImageViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (original != null)
                {
                    ...
                    var oldFilepath = _userService.GetUser(User.Identity.GetUserId()).ProfilePicUrl;
                    ...
                    _userService.SaveImageURL(User.Identity.GetUserId(), "~/Content/ProfilePics/" + fileName + ".png");
        ...
        public ViewResult UserProfile(string id)
        {
            ...
            if (_followRequestService.RequestSent((User.Identity.GetUserId()), id))
            {
                userprofile.RequestSent = true;
            }
            if (_followUserService.IsFollowing(currentuserid, id))
            {
        ...
        {
            var user = _userProfileService.GetUser(User.Identity.GetUserId());
            var editUser = Mapper.Map<UserProfile, UserProfileFormModel>(user);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        {
            var user = Mapper.Map<UserProfileFormModel, UserProfile>(editedProfile);
            var applicationUser = _userService.GetUser(editedProfile.UserId);
            ...
            if (ModelState.IsValid)
            {
                _userService.UpdateUser(applicationUser);
        ...
        public ActionResult FollowRequest(string id)
        {
            var followRequestFormModel = new FollowRequestFormModel
            {
                ...
                ToUserId = id,
                FromUser = _userService.GetUser(User.Identity.GetUserId()),
                ToUser = _userService.GetUser(id)
            };
            var followRequest = Mapper.Map<FollowRequestFormModel, FollowRequest>(followRequestFormModel);
            _followRequestService.CreateFollowRequest(followRequest);
        ...
        public ActionResult AcceptRequest(string touserid, string fromuserid)
        {
            var newFollowUser = new FollowUser
            {
                ...
                ToUserId = touserid,
                FromUser = _userService.GetUser(fromuserid),
                ToUser = _userService.GetUser(touserid)
            };
            _followUserService.CreateFollowUserFromRequest(newFollowUser, _followRequestService);
            return RedirectToAction("Index", "Notification");
        }

        public ActionResult RejectRequest(string toUserId, string fromuserId)
        {
            _followRequestService.DeleteFollowRequest(fromuserId, toUserId);
            return RedirectToAction("Index", "Notification");
        }

        public ActionResult Unfollow(string id)
        {
            _followUserService.DeleteFollowUser(id, User.Identity.GetUserId());
        ...
        {
            var users = _followUserService.GetFollowers(User.Identity.GetUserId(), page, 10);
        ...
        {
            var users = _followUserService.GetFollowings(User.Identity.GetUserId(), page, 10);
